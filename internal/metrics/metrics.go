...
var (
	...
	requestsTotal = promauto.NewCounterVec( // NewCounterVec - New_новая Vec_таблица, для счётчика типа Counter Vec - таблица состоящая из заданных ярлыков и их количества, если нужно например узнать операция какого типа была совершена и удачно или с ошибкой, то как в этом случае, в конце указываем что каждый раз, когда я буду менять(добавлять в) эту переменную, обязательно должны передаваться 2 ярлыка []string{"method", "status"}
		...
		[]string{"method", "status"}, // указываем ярлыки, которы обязательно должен получить прометеус, что бы обновить счётчик
	)
